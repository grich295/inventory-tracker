/* Inventory Tracker v8.5.7 CLEAN
...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .qty-stepper-down-857{background:#c62828;}
      .qty-stepper-up-857{background:#218838;}
      .qty-stepper-btn-857:active{transform:translateY(1px);box-shadow:none;}
      .qty-stepper-btn-857:disabled{opacity:.45;}
    `;
  ...
  function enhance() {
    ...
    if (!form || !input || input.dataset.stepper857) return;

    const type = String(form.dataset.type || '').toUpperCase();
    if (type !== 'ADD' && type !== 'USE') return;

    input.dataset.stepper857 = '1';

    const wrap = document.createElement('div');
    wrap.className = 'qty-stepper-857';
    input.parentNode.insertBefore(wrap, input);
    wrap.appendChild(input);

    const down = document.createElement('button');
    down.type = 'button';
    down.className = 'qty-stepper-btn-857 qty-stepper-down-857';
    down.textContent = '▼';
    ...
    up.type = 'button';
    up.className = 'qty-stepper-btn-857 qty-stepper-up-857';
    up.textContent = '▲';
