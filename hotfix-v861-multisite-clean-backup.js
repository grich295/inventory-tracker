/* Inventory Tracker v8.6.1 CLEAN - Multi-site clean start
...
(() => {
  'use strict';
  if(window.__INVENTORY_MULTISITE_V861)return;
  window.__INVENTORY_MULTISITE_V861=true;
  ...
  function flash(msg,type='success'){
    let d=document.getElementById('inventorySiteFlashV861');
    if(!d){d=document.createElement('div');d.id='inventorySiteFlashV861';Object.assign(d.style,{position:'fixed',left:'16px',right:'16px',bottom:'16px',zIndex:'99999',padding:'12px 14px',borderRadius:'10px',boxShadow:'0 6px 24px rgba(0,0,0,.18)'});document.body.appendChild(d)}
  ...
  function decorateVersion(){
    document.querySelectorAll('.app-version-badge').forEach(x=>x.textContent='v8.6.1');
  }

  function decorateSwitcher(){
    if(!session||!loaded)return;
    const top=document.querySelector('.top-actions');if(!top)return;
    let wrap=document.getElementById('inventorySiteSwitcherV861');
    if(!wrap){
      wrap=document.createElement('div');wrap.id='inventorySiteSwitcherV861';wrap.style.display='flex';wrap.style.gap='.4rem';wrap.style.alignItems='center';
      top.prepend(wrap);
    }
    const rows=allowedSites();
    wrap.innerHTML=`<label style="display:flex;align-items:center;gap:.35rem;margin:0"><span style="font-size:.78rem;opacity:.78">Site</span><select id="inventorySiteSelectV861" style="max-width:190px">${rows.map(s=>`<option value="${esc(s.id)}" ${s.id===currentSiteId?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label>`;
    const sel=document.getElementById('inventorySiteSelectV861');
    if(sel)sel.onchange=()=>switchSite(sel.value);
  }

  function siteCardHtml(){
    return `<div class="card" id="inventorySitesV861">
      <div class="row-between"><div><h2>Sites</h2><p class="muted">New sites start clean. Inventory gets only default settings/categories; Energy gets a blank site shell. No stock, history, invoices, readings or other operational records are copied.</p></div><button class="btn" type="button" data-v861-create-site>New site</button></div>
      <div class="card-list">${sites.filter(s=>s.active!==false).map(s=>{
        ...
        return `<div class="item-card compact"><div class="row-between"><div><strong>${esc(s.name)}</strong><div class="meta"><span class="badge ${current?'good':''}">${current?'CURRENT':'Site'}</span><span>Inventory: ${esc(inv?.status||'READY')}</span><span>Energy: ${esc(en?.status||'READY')}</span></div><small>${esc(inv?.note||'')}</small></div>${s.id!==currentSiteId?`<button class="btn secondary small" type="button" data-v861-open-site="${esc(s.id)}">Open Inventory</button>`:''}</div></div>`;
      }).join('')}</div>
  ...
  function decorateSitesAdmin(){
    ...
    const anchor=people||usersHeading;if(!anchor||document.getElementById('inventorySitesV861'))return;
  ...
  function decorateBackup(){
    ...
    const card=h?.closest('.card');if(!card||card.querySelector('.site-backup-note-v861'))return;
    const d=document.createElement('div');d.className='notice success site-backup-note-v861';
  ...
  async function createSite(){
    ...
    if(error)return flash(error.message,'error');
    await loadState();
    document.getElementById('inventorySitesV861')?.remove();
  ...
  function decorateAccessModal(){
    if(!isAdmin())return;
    const m=document.getElementById('paV860Modal');if(!m||m.dataset.v861Sites==='1')return;
    const save=m.querySelector('[data-pa-save]');if(!save)return;
    const uid=save.dataset.paSave;m.dataset.v861Sites='1';
    ...
    const checks=(module,home)=>sites.filter(s=>s.active!==false).map(s=>{
      ...
      return `<label style="display:flex;gap:.45rem;align-items:center"><input class="v861-extra-site" data-module="${module}" value="${esc(s.id)}" type="checkbox" ${checked?'checked':''} ${s.id===home?'disabled':''}>${esc(s.name)}${s.id===home?' · Home':''}</label>`;
    }).join('');
    ...
    save.addEventListener('click',()=>{
      const snapshot={
        ...
        selected:[...m.querySelectorAll('.v861-extra-site:checked')].map(x=>({module:x.dataset.module,site:x.value}))
      };
      setTimeout(async()=>{
  ...
  document.addEventListener('click',e=>{
    const n=e.target.closest?.('[data-v861-create-site]');if(n){e.preventDefault();createSite();return}
    const o=e.target.closest?.('[data-v861-open-site]');if(o){e.preventDefault();switchSite(o.dataset.v861OpenSite);return}
    queueDecorate();
  },true);
  ...
  })().catch(e=>console.warn('Inventory multi-site v8.6.1',e));
})();
